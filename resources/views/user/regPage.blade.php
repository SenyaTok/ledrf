...
@section('body')
    ...
    <div class="border border-secondary rounded m-2 p-3">
        ...
        <form action="{{ @route('signUp') }}" method="POST" class="form-auth">
            @csrf
            <h3>Регистрация</h3>
            <div class="form-block-wrapper"><input type="text" name="login" placeholder="Фамилия"
                    pattern="[A-Za-z0-9_]{3,16}" required></div>
            <div class="form-block-wrapper"><input type="text" name="login" placeholder="Имя"
                    pattern="[A-Za-z0-9_]{3,16}" required></div>
            <div class="form-block-wrapper"><input type="text" name="login" placeholder="Телефон"
                    pattern="[A-Za-z0-9_]{3,16}" required></div>
            <div class="form-block-wrapper"><input type="email" name="email" placeholder="Электронная почта" required>
            </div>
